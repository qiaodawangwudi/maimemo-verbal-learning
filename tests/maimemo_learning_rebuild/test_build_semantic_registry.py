...
class LessonFiveOverrideTests(unittest.TestCase):
    ...
    def test_dictionary_supplement_does_not_claim_teacher_authorship(self):
        ...
        result = apply_lesson_five_override(record, override, "dictionary-batch")

        self.assertEqual("secondary_reference", result["source_kind"])
        self.assertEqual(override["evidence"], result["evidence"])
        self.assertEqual("dictionary-batch", result["provenance"]["manual_semantic_review"])
...
class FourPoemsOverrideTests(unittest.TestCase):
    def test_manual_evidence_replaces_truncated_extracted_evidence(self):
        ...
        result = apply_four_poems_override(record, override, "batch20")
        ...
        self.assertNotIn("candidate", result)
        self.assertNotIn("review_blockers", result)
        self.assertEqual("batch20", result["provenance"]["manual_semantic_review"])
